...
def main():
    print("=== DEVAI (PyTorch) | Kasım K-Fold ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("Device:", device)

    ds = get_dataset()

    # Abdullah gibi: ikisi de K-Fold
    run_kfold("Kasım - Model1 (ResNet18 Transfer)", create_kasim_m1, ds, device, epochs=1)
    run_kfold("Kasım - Model2 (SE-Attention CNN)", create_kasim_m2, ds, device, epochs=1)
